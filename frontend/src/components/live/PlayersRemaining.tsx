      ...
      <div className="flex items-center justify-center gap-6">
        ...
        <div className="relative w-20 h-20">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            ...
            <span className="text-sm text-foreground-dim uppercase">Left</span>
          </div>
        </div>
        ...
        <div className="space-y-2">
          <div className="flex items-baseline gap-2">
            ...
            <span className="text-sm text-foreground-dim uppercase">Finished</span>
          </div>

          {avgPlayersLeft !== null && (
            <div className="flex items-baseline gap-2">
              <span className="text-lg text-foreground-muted">{avgPlayersLeft.toFixed(1)}</span>
              <span className="text-sm text-foreground-dim uppercase">10K Avg</span>
            </div>
          )}

          {/* Advantage indicator */}
          {diff !== null && Math.abs(diff) >= 0.5 && (
            <div className={`text-sm ${diff > 0 ? 'text-tt-green' : 'text-destructive'}`}>
            ...
            {playersLeft.map((player) => {
              ...
              return (
                ...
                >
                  {position && (
                    <span
                      className={`text-sm uppercase ${isPlaying ? 'text-tt-green/70' : 'text-foreground-dim'}`}
                    >
                      {position}
                    </span>
                  )}
