    ...
    <div className="space-y-6">
        ...
        <>
          ...
          {isLoadingOptimize && (
            <div className="text-center py-12 text-gray-400">
              Calculating optimal transfers across multiple gameweeks...
            </div>
          )}
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="p-4 bg-gray-800 rounded-lg">
                  <div className="text-gray-400 text-sm">Squad Value</div>
                  <div className="text-2xl font-bold text-white">£{optimizeData.current_squad.squad_value.toFixed(1)}m</div>
                </div>
                <div className="p-4 bg-gray-800 rounded-lg">
                  <div className="text-gray-400 text-sm">Bank</div>
                  <div className="text-2xl font-bold text-white">£{optimizeData.current_squad.bank.toFixed(1)}m</div>
                </div>
                <div className="p-4 bg-gray-800 rounded-lg">
                  <div className="text-gray-400 text-sm">Free Transfers</div>
                  <div className="text-2xl font-bold text-white">{freeTransfers}</div>
                </div>
                <div className="p-4 bg-gray-800 rounded-lg">
                  <div className="text-gray-400 text-sm">Predicted (Next {optimizeData.planning_horizon.length} GWs)</div>
                  <div className="text-2xl font-bold text-green-400">{optimizeData.current_squad.predicted_points.total} pts</div>
                </div>
              </div>

              {/* Predicted Points by Gameweek */}
              <div className="bg-gray-800 p-4 rounded-lg">
                <h3 className="text-lg font-semibold text-white mb-3">Predicted Points by Gameweek</h3>
                <div className="flex gap-2 flex-wrap">
                  {optimizeData.planning_horizon.map(gw => {
                    ...
                    return (
                      <div key={gw} className={`px-4 py-3 rounded-lg ${hasDgw ? 'bg-green-600/30 border border-green-500' : 'bg-gray-700'}`}>
                        <div className="text-xs text-gray-400">GW{gw}{hasDgw && ' (DGW)'}</div>
              ...
              {/* Chip Planning */}
              <div className="bg-gray-800 p-4 rounded-lg">
                <h3 className="text-lg font-semibold text-white mb-4">Chip Planning</h3>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {(['wildcard', 'bench_boost', 'free_hit', 'triple_captain'] as const).map(chip => {
                    ...
                    return (
                      <div key={chip} className="space-y-2">
                        <label className="text-white text-sm">{chipLabels[chip]}</label>
                        <select
                          value={fixedGw ?? ''}
              ...
              <div className="bg-gray-800 p-4 rounded-lg">
                <h3 className="text-lg font-semibold text-white mb-4">Recommended Transfers</h3>
                <div className="space-y-4">
                  {optimizeData.recommendations.map((rec, idx) => (
                    <div
                      key={idx}
                      className={`p-4 rounded-lg border ${rec.recommended ? 'border-green-500 bg-green-600/10' : 'border-gray-600 bg-gray-700/50'}`}
                    >
                      <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                        <div className="flex items-center gap-4 flex-1">
                          ...
                          <div className="text-center min-w-[100px]">
                            ...
                            <div className="text-white font-bold">{rec.out.web_name}</div>
                            <div className="text-gray-400 text-sm">{teamsMap.get(rec.out.team)}</div>
                            <div className="text-gray-500 text-xs">£{rec.out.price.toFixed(1)}m • {rec.out.total_predicted} pts</div>
                          </div>
                          ...
                          {/* In player */}
                          <div className="text-center min-w-[100px]">
                            <div className="text-green-400 text-xs mb-1">IN</div>
                            <div className="text-white font-bold">{rec.in.web_name}</div>
                            <div className="text-gray-400 text-sm">{teamsMap.get(rec.in.team)}</div>
                            <div className="text-gray-500 text-xs">£{rec.in.price.toFixed(1)}m • {rec.in.total_predicted} pts</div>
                          </div>
                        </div>

                        {/* Gain summary */}
                        <div className="text-right">
                          <div className={`text-2xl font-bold ${rec.net_gain > 0 ? 'text-green-400' : 'text-red-400'}`}>
      ...
      {viewMode === 'optimize' && !managerId && (
        <div className="text-center py-12 text-gray-500">
          Enter your FPL Manager ID to get multi-week transfer optimization
        </div>
      )}
    </div>
  )
